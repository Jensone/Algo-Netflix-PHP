...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
   $nouvelUtilisateur = 'Nouveau';
   $filmNote = $_POST['film'];
   $note = $_POST['note'];

   $notesNouvelUtilisateur = [
      $filmNote => $note
   ];
   ...
   foreach ($utilisateurs as $utilisateur) {
      foreach ($notesUtilisateurs[$utilisateur] as $film => $note) {
         if (array_key_exists($film, $notesNouvelUtilisateur)) {
            $utilisateursSimilaires[$utilisateur][] = $film;
         }
      }
   }

   // Recherche des notes des utilisateurs similaires
   $notesUtilisateursSimilaires = [];
   foreach ($utilisateursSimilaires as $utilisateur => $films) {
      foreach ($notesUtilisateurs[$utilisateur] as $film => $note) {
         if ($note > 0 && $film != $filmNote) { // Vérifier que le film n'est pas celui noté par le nouvel utilisateur
            $notesUtilisateursSimilaires[$utilisateur][] = $film;
         }
      }
   }

   // Classement par films les plus regardés par les utilisateurs similaires
   foreach ($notesUtilisateursSimilaires as $utilisateur => $films) {
      foreach ($films as $film) {
         if (array_key_exists($film, $recommandations)) {
            $recommandations[$film]++;
         } else {
            $recommandations[$film] = 1;
         }
      }
   }
   ...
   // Slicing $recommandations to 3 data only
   $recommandations = array_slice($recommandations, 0, 3);
} // Fin du traitement des données du nouvel utilisateur
...
<body>
   <main class="container">
      ...
      <p>Voici les 3 films que nous vous recommandons :</p>
      <ul>
         <?php
         if (!empty($recommandations)) {
            foreach ($recommandations as $film => $note) {
               echo "<li>" . $film . "</li>";
            }
         } else {
